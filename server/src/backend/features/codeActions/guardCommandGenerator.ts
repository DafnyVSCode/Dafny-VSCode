...
export class GuardCommandGenerator extends BaseCommandGenerator {
    ...
    private addGuards(guardKeyWord: string, symbols: Symbol[]): void {
        ...
        const insertPosition: Position = this.findGuardInsertPosition(definingMethod);
    ...
    private findGuardInsertPosition(definingMethod: Symbol): Position {
        let insertPosition: Position = this.dummyPosition;
        if(definingMethod) {
            insertPosition = this.documentDecorator.findInsertionPointOfContract(definingMethod.start, this.diagnostic.range.start);
        }
        if(!insertPosition || insertPosition === this.dummyPosition) {
